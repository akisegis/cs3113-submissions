...
enum EntityType { PLAYER, ENEMY, COLLECT };
enum AIType     { GUARD, COLLECTABLE };
...
private:
   ...
   // int atk_timer = 0,
    ...
    int m_walking[7][4]; // 5x4 array for player animations (walking,attacking)
...
public:
    ...
    // ————— METHODS ————— //
    Entity();
    Entity(GLuint texture_id, float speed, glm::vec3 acceleration, float jump_power, int walking[7][4], float animation_time,
   ...
   // void attacking() { m_animation_indices = m_walking[ATTACK]; }
   // void atk()       { attacking(); m_is_atk = true;            }
    ...
    //int       const get_enemy_num()     const { return enemy_num;       }
    ...
    void set_walking(int walking[7][4])
    {
        for (int i = 0; i < 5; ++i)
        {
            for (int j = 0; j < 4; ++j)
